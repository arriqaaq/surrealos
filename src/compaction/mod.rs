...
///
/// Note: There is no `Beat` variant. Since SurrealOS's compaction is synchronous
...
impl LiveCompaction {
	...
	async fn drain_remaining(&mut self) -> Result<()> {
		while let Some((key, value)) = self.iterator.advance().await? {
			if let Some(ref mut writer) = self.writer {
				writer.add(key, &value)?;
			}
			self.quotas.record_entry();
	...
	async fn commit(mut self, opts: &CompactionOptions) -> Result<()> {
		...
		cleanup_old_tables(&opts.table_store, &self.input, opts.branching_enabled).await;
...
pub(crate) struct CompactionScheduler {
	...
	/// Local compactor epoch for fencing (0 = no fencing)
...
impl CompactionScheduler {
	...
	pub(crate) fn new(
		...
		let branching_enabled = opts.branch_name.is_some();
		Self {
			beats_per_bar,
	...
	async fn complete_half_bar(&mut self) -> Result<()> {
		...
		if !pending_ops.is_empty() {
			...
			if self.local_compactor_epoch > 0 && levels.compactor_epoch > self.local_compactor_epoch
			{
...
// ============================================================================
// Test-only accessors and unit tests
// ============================================================================

#[cfg(test)]
impl CompactionScheduler {
	pub(crate) fn current_op(&self) -> u64 {
		self.op
	}

	pub(crate) fn get_beats_per_bar(&self) -> u64 {
		self.beats_per_bar
	}

	pub(crate) fn get_half_bar(&self) -> u64 {
		self.half_bar
	}

	pub(crate) fn active_compaction_count(&self) -> usize {
		self.active_compactions.len()
	}

	pub(crate) fn is_level_active(&self, level_b: u8, beat: u64) -> bool {
		self.level_active(level_b, beat)
	}
}
...
mod tests {
	...
	// ── CompactionQuotas unit tests ──

	#[test]
	fn test_quotas_basic_distribution() {
		// 100 entries distributed across 10 beats should give 10 per beat
		let mut q = CompactionQuotas::new(100);
		assert!(!q.bar_exhausted());
		assert!(!q.has_output());

		for beats_remaining in (1..=10).rev() {
			q.commence_beat(beats_remaining);
			assert_eq!(q.beat, 10, "Each beat should get ceil(remaining/beats_remaining) = 10");
			assert_eq!(q.beat_done, 0);
		...
		assert!(q.bar_exhausted());
		assert_eq!(q.bar_done, 100);
		assert!(q.has_output());
	}

	#[test]
	fn test_quotas_uneven_distribution() {
		// 7 entries across 3 beats: ceil(7/3)=3, ceil(4/2)=2, ceil(2/1)=2
		let mut q = CompactionQuotas::new(7);

		// Beat 1: 3 beats remaining → ceil(7/3) = 3
		q.commence_beat(3);
		assert_eq!(q.beat, 3);
		for _ in 0..3 {
			q.record_entry();
		...
		assert!(q.beat_exhausted());
		assert_eq!(q.bar_done, 3);

		// Beat 2: 2 beats remaining → ceil(4/2) = 2
		q.commence_beat(2);
		assert_eq!(q.beat, 2);
		for _ in 0..2 {
			q.record_entry();
		...
		assert!(q.beat_exhausted());
		assert_eq!(q.bar_done, 5);

		// Beat 3: 1 beat remaining → ceil(2/1) = 2
		q.commence_beat(1);
		assert_eq!(q.beat, 2);
		for _ in 0..2 {
			q.record_entry();
		...
		assert!(q.beat_exhausted());
		assert_eq!(q.bar_done, 7);
		assert!(q.bar_exhausted());
	}

	#[test]
	fn test_quotas_zero_remaining() {
		// bar=0 means no work — should be immediately exhausted
		let mut q = CompactionQuotas::new(0);
		assert!(q.bar_exhausted());
		assert!(!q.has_output());
	...
	#[test]
	fn test_quotas_has_output() {
		let mut q = CompactionQuotas::new(10);
		assert!(!q.has_output());
	...
	#[test]
	fn test_quotas_beat_resets_each_commence() {
		let mut q = CompactionQuotas::new(20);
		...
		assert_eq!(q.beat_done, 10);
		assert_eq!(q.bar_done, 10);

		// Second beat — beat_done resets but bar_done persists
		q.commence_beat(1);
		assert_eq!(q.beat_done, 0);
		assert_eq!(q.bar_done, 10);
		for _ in 0..10 {
		...
		assert_eq!(q.beat_done, 10);
		assert_eq!(q.bar_done, 20);
		assert!(q.bar_exhausted());
	}

	#[test]
	fn test_quotas_saturating_sub() {
		// Test that bar_done exceeding bar (imprecise estimate) is handled
		let mut q = CompactionQuotas::new(3);

		q.commence_beat(1);
		// Process more entries than estimated bar quota
		for _ in 0..5 {
			q.record_entry();
		}

		// bar_done (5) >= bar (3) — should be exhausted
		assert!(q.bar_exhausted());
		assert_eq!(q.bar_done, 5);

		// commence_beat with overshot bar_done — saturating_sub prevents underflow
		q.commence_beat(1);
		assert_eq!(q.beat, 0); // remaining = 3 - 5 saturated to 0
		assert!(q.beat_exhausted());
	}
}
