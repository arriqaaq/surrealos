...
pub(crate) struct LevelManifest {
	...
	/// Incremented via CAS when a new writer takes over.
	pub(crate) writer_epoch: u64,

	/// Monotonically increasing compactor epoch for fencing.
	/// Incremented via CAS when a new compactor takes over.
	pub(crate) compactor_epoch: u64,

	/// Identity of the current leader replica.
	/// Set by the external coordination layer (e.g., Raft).
	/// 0 means single-node mode (no distributed coordination).
	pub(crate) leader_id: u128,
}

impl LevelManifest {
	pub(crate) async fn new(opts: Arc<Options>, table_store: Arc<CloudStore>) -> Result<Self> {
