...
use crate::compaction::{CompactionChoice, CompactionScheduler, CompactionStrategy};
...
impl TestEnv {
	...
	fn new_with_levels(level_count: u8) -> Self {
		...
		let table_dir = temp_dir.path().join("sst");
	...
	async fn create_test_table(
		...
		// Write entries to an in-memory buffer via TableWriter
		let mut buf = Vec::new();
		let mut writer = TableWriter::new(&mut buf, sst_id, Arc::clone(&self.options), 0);
		...
		// Build the table from the buffer using the async test helper
		let table = new_test_table(sst_id, Arc::clone(&self.options), buf).await?;
...
async fn create_test_manifest(
	...
	for (level_idx, level_specs) in level_tables.iter().enumerate() {
		for &(id, min_seq, min_key, max_key) in level_specs {
			...
			let table = env.create_test_table(id, entries).await?;
	...
	let mut manifest = LevelManifest {
		manifest_dir: env.options.manifest_dir(),
		manifest_id: 0,
		levels,
		hidden_set: HashSet::new(),
		manifest_format_version: crate::manifest::MANIFEST_FORMAT_VERSION_V3,
		snapshots: Vec::new(),
		log_number: 0,
		last_sequence: 0,
		writer_epoch: 0,
...
async fn verify_keys_after_compaction(
	...
	let levels = {
		...
		manifest_guard.levels.get_levels().clone()
	};
...
async fn verify_all_keys_present(
	manifest: &RwLock<LevelManifest>,
...
async fn perform_compaction_rounds(compactor: &Compactor, rounds: usize) {
	for i in 1..=rounds {
...
async fn test_level_selection() {
	...
	let manifest = create_test_manifest(&env, level_tables).await.unwrap();
	...
	match choice {
		CompactionChoice::Merge(input) | CompactionChoice::Move(input) => {
			...
			for id in 1u64..=5 {
				assert!(
					input.tables_to_merge.contains(&test_sst_id(id)),
			...
			assert!(
				input.tables_to_merge.contains(&test_sst_id(12)),
...
async fn test_compaction_edge_cases() {
	...
	let manifest = create_test_manifest(&env, empty_level_tables).await.unwrap();
	let opts = create_options_with_compaction_settings(&env.options, 4, 2.0);
	let strategy = Strategy::from_options(opts);
	...
	let manifest = create_test_manifest(&env, last_level_tables).await.unwrap();
...
#[test(tokio::test)]
async fn test_level_selection_score_based() {
	let env = TestEnv::new();
	...
	let l1_table = env.create_test_table(1, l1_entries).await.unwrap();
	...
	let l2_table = env.create_test_table(2, l2_entries).await.unwrap();
	...
	write_manifest_to_disk(&mut manifest).unwrap();
...
async fn test_simple_merge_compaction() {
	...
	for i in 0..TABLE_COUNT {
		...
		let table = env.create_test_table(id, entries).await.unwrap();
	...
	let (_, all_key_values) = verify_keys_after_compaction(&manifest, &expected_keys).await;
...

	{
		...
		let original_table_ids: Vec<SstId> = (1..=TABLE_COUNT as u64).map(test_sst_id).collect();
...
#[test(tokio::test)]
async fn test_select_tables_for_compaction_bug() {
	let env = TestEnv::new();
...
#[test(tokio::test)]
async fn test_l1_to_l2_table_selection() {
	let env = TestEnv::new();
	let opts = create_options_with_compaction_settings(&env.options, 4, 10.0);
	let strategy = Strategy::from_options(opts);
	...
	assert!(
		!selected_tables.contains(&test_sst_id(1))
			|| !selected_tables.contains(&test_sst_id(2))
			|| !selected_tables.contains(&test_sst_id(3)),
...
#[test(tokio::test)]
async fn test_l1_compaction_bounds_correctness() {
	...
	assert_eq!(
		source_level.tables[0].id,
		test_sst_id(1),
		"First table should be table1 (smallest key)"
	);
	...
	assert!(
		selected_l2_ids.contains(&test_sst_id(11)),
		"Should select L2 table 11 (overlaps with selected table3), but selected: {:?}",
		selected_l2_ids
	);
	assert!(
		!selected_l2_ids.contains(&test_sst_id(10)),
		"Should NOT select L2 table 10 (overlaps with table1, not selected table3)"
	);
	assert!(
		!selected_l2_ids.contains(&test_sst_id(12)),
		"Should NOT select L2 table 12 (no overlap)"
	);
}
...
async fn test_compaction_with_large_keys_and_values() {
	...
	let large_table = env.create_test_table(1, large_entries.clone()).await.unwrap();
	...
	// Run compaction
	perform_compaction_rounds(&compactor, 2).await;

	// Verify all large keys and values are preserved
	assert!(
		verify_all_keys_present(&manifest, &expected_data).await,
...
async fn test_compaction_respects_sequence_numbers() {
	...
	for i in 0..5 {
		...
		// Create the table
		let table = env.create_test_table(i as u64 + 1, entries).await.unwrap();
		Arc::make_mut(&mut levels.get_levels_mut()[0]).insert(table);
	}
...
async fn test_tombstone_propagation() {
	...
	let table = env.create_test_table(100, all_entries).await.unwrap();
	Arc::make_mut(&mut levels.get_levels_mut()[0]).insert(table);
...
async fn test_l0_overlapping_keys_compaction() {
	...
	let table1 = env.create_test_table(1, entries1).await.unwrap();
	let table2 = env.create_test_table(2, entries2).await.unwrap();
	let table3 = env.create_test_table(3, entries3).await.unwrap();
...
async fn test_tombstone_propagation_through_levels() {
	...
	for table_idx in 0..4 {
		...
		let table = env.create_test_table(100 + table_idx, l2_entries).await.unwrap();
	...
	let l3_table = env.create_test_table(200, l3_entries).await.unwrap();
...
#[test(tokio::test)]
async fn test_tombstone_propagation_journey() {
	let env = TestEnv::new();
	...
	let tombstone_table = env.create_test_table(100, tombstone_entries).await.unwrap();
	...
	let value_table = env.create_test_table(101, value_entries).await.unwrap();
	...
	while let Some(entry) = comp_iter_bottom.advance().await.unwrap() {
		bottom_result.push(entry);
	}

	// Bottom level should filter out tombstones
	let has_tombstones = bottom_result.iter().any(|(key, _)| key.is_tombstone());
...
#[test(tokio::test)]
async fn test_table_properties_population() {
	let env = TestEnv::new();
	...
	let table_id = 11u64;
	let table = env.create_test_table(table_id, entries).await.unwrap();

	let meta = &table.meta;
	let props = &meta.properties;

	// Verify Properties fields
	assert_eq!(props.id, test_sst_id(table_id));
	...
	assert_eq!(selected, Some(test_sst_id(table_id)));
}

#[test(tokio::test)]
async fn test_score_based_level_selection() {
	let env = TestEnv::new();
	// Ensure manifest directory exists
	std::fs::create_dir_all(env.options.manifest_dir()).unwrap();
	let mut manifest =
		LevelManifest::new(Arc::clone(&env.options), test_table_store()).await.unwrap();
	...
	for i in 0..5 {
		...
		let table = env.create_test_table(i as u64, entries).await.unwrap();
	...
	for i in 0..3 {
		...
		let table = env.create_test_table((10 + i) as u64, entries).await.unwrap();
...
#[test(tokio::test)]
async fn test_bytes_based_level_limits() {
	let env = TestEnv::new();
	// Ensure manifest directory exists
	std::fs::create_dir_all(env.options.manifest_dir()).unwrap();
	let mut manifest =
...
#[test(tokio::test)]
async fn test_bottom_level_compaction() {
	...
	for i in 0..3 {
		...
		let table = env.create_test_table((20 + i) as u64, entries).await.unwrap();
...
async fn create_test_table_with_bounds(
	opts: &Arc<Options>,
	id: u64,
	smallest_key: &[u8],
	largest_key: &[u8],
	...
	let sst_id = test_sst_id(id);
	let mut buf = Vec::new();
	let mut writer = crate::sstable::table::TableWriter::new(&mut buf, sst_id, Arc::clone(opts), 0);
	...
	let table = new_test_table(sst_id, Arc::clone(opts), buf).await.unwrap();
	Arc::new(table)
}

#[test(tokio::test)]
async fn test_combined_key_range_single_table() {
	let temp_dir = TempDir::new().unwrap();
	...
	let table = create_test_table_with_bounds(&opts, 1, b"a", b"z").await;
	level.insert(table);

	let mut ids = HashSet::new();
	ids.insert(test_sst_id(1));
...
#[test(tokio::test)]
async fn test_combined_key_range_multiple_tables() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let mut level = Level::with_capacity(10);
	let table1 = create_test_table_with_bounds(&opts, 1, b"a", b"m").await;
	let table2 = create_test_table_with_bounds(&opts, 2, b"g", b"z").await;
	level.insert(table1);
	level.insert(table2);

	let mut ids = HashSet::new();
	ids.insert(test_sst_id(1));
	ids.insert(test_sst_id(2));
...
#[test(tokio::test)]
async fn test_combined_key_range_overlapping_tables() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let mut level = Level::with_capacity(10);
	let table1 = create_test_table_with_bounds(&opts, 1, b"a", b"m").await;
	let table2 = create_test_table_with_bounds(&opts, 2, b"m", b"t").await;
	let table3 = create_test_table_with_bounds(&opts, 3, b"t", b"v").await;
	level.insert(table1);
	level.insert(table2);
	level.insert(table3);
...
#[test(tokio::test)]
async fn test_combined_key_range_empty_ids() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let ids: HashSet<SstId> = HashSet::new();
...
#[test(tokio::test)]
async fn test_select_overlapping_ranges_no_overlap() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let mut level = Level::with_capacity(10);
	let table1 = create_test_table_with_bounds(&opts, 1, b"a", b"b").await;
	let table2 = create_test_table_with_bounds(&opts, 2, b"d", b"e").await;
	let table3 = create_test_table_with_bounds(&opts, 3, b"g", b"h").await;
	level.insert(table1);
	level.insert(table2);
	level.insert(table3);

	let result = Strategy::select_overlapping_ranges(&level, test_sst_id(2)).unwrap();
	assert_eq!(result.len(), 1);
	assert!(result.contains(&test_sst_id(2)));
}

#[test(tokio::test)]
async fn test_select_overlapping_ranges_direct_overlap() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let mut level = Level::with_capacity(10);
	let table1 = create_test_table_with_bounds(&opts, 1, b"a", b"c").await;
	let table2 = create_test_table_with_bounds(&opts, 2, b"c", b"e").await;
	level.insert(table1);
	level.insert(table2);

	let result = Strategy::select_overlapping_ranges(&level, test_sst_id(1)).unwrap();
	assert_eq!(result.len(), 2);
	assert!(result.contains(&test_sst_id(1)));
	assert!(result.contains(&test_sst_id(2)));
}

#[test(tokio::test)]
async fn test_select_overlapping_ranges_chain() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let mut level = Level::with_capacity(10);
	let table1 = create_test_table_with_bounds(&opts, 1, b"a", b"b").await;
	let table2 = create_test_table_with_bounds(&opts, 2, b"b", b"c").await;
	let table3 = create_test_table_with_bounds(&opts, 3, b"c", b"d").await;
	let table4 = create_test_table_with_bounds(&opts, 4, b"e", b"f").await;
	level.insert(table1);
	level.insert(table2);
	level.insert(table3);
	level.insert(table4);
	...
	assert!(result.contains(&test_sst_id(1)));
	assert!(result.contains(&test_sst_id(2)));
	assert!(result.contains(&test_sst_id(3)));
	assert!(!result.contains(&test_sst_id(4)));
}

#[test(tokio::test)]
async fn test_select_overlapping_ranges_full_overlap() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let mut level = Level::with_capacity(10);
	let table1 = create_test_table_with_bounds(&opts, 1, b"a", b"z").await;
	let table2 = create_test_table_with_bounds(&opts, 2, b"b", b"c").await;
	level.insert(table1);
	level.insert(table2);
...
#[test(tokio::test)]
async fn test_select_overlapping_ranges_adjacent_no_overlap() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let mut level = Level::with_capacity(10);
	let table1 = create_test_table_with_bounds(&opts, 1, b"a", b"b").await;
	let table2 = create_test_table_with_bounds(&opts, 2, b"c", b"d").await;
	level.insert(table1);
	level.insert(table2);
...
#[test(tokio::test)]
async fn test_expand_same_user_key_different_seq() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let sst_id1 = test_sst_id(1);
	let mut buf1 = Vec::new();
	let mut writer1 =
		crate::sstable::table::TableWriter::new(&mut buf1, sst_id1, Arc::clone(&opts), 0);
	...
	let table1 = new_test_table(sst_id1, Arc::clone(&opts), buf1).await.unwrap();
	level.insert(Arc::new(table1));

	// File 2: user_key="foo" at seq=50 (older version)
	let sst_id2 = test_sst_id(2);
	let mut buf2 = Vec::new();
	let mut writer2 =
		crate::sstable::table::TableWriter::new(&mut buf2, sst_id2, Arc::clone(&opts), 0);
	let key2 = InternalKey::new(b"foo".to_vec(), 50, InternalKeyKind::Set, 0);
	writer2.add(key2, &value).unwrap();
	writer2.finish().unwrap();
	let table2 = new_test_table(sst_id2, Arc::clone(&opts), buf2).await.unwrap();
...
#[test(tokio::test)]
async fn test_select_overlapping_ranges_nonexistent_id() {
	let temp_dir = TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		..Default::default()
	...
	let result = Strategy::select_overlapping_ranges(&level, test_sst_id(999));
	assert!(result.is_err());
	match result {
		Err(crate::error::Error::TableNotFound(id)) => assert_eq!(id, test_sst_id(999)),
...
#[test(tokio::test)]
async fn test_clean_cut_shared_boundary_key() {
	let env = TestEnv::new();
	...
	let table1 = env.create_test_table(1, file1_entries).await.unwrap();
	...
	let table2 = env.create_test_table(2, file2_entries).await.unwrap();
	...
	let table3 = env.create_test_table(3, file3_entries).await.unwrap();
	...
	let selected = Strategy::select_overlapping_ranges(source_level, test_sst_id(1)).unwrap();

	// Verify file 1 and file 2 are both included (shared boundary "foo")
	assert!(selected.contains(&test_sst_id(1)), "File 1 should be included (initially selected)");
	assert!(
		selected.contains(&test_sst_id(2)),
		"File 2 should be included (shares boundary key 'foo' with file 1)"
	);
	// Verify file 3 is NOT included (no shared boundary)
	assert!(
		!selected.contains(&test_sst_id(3)),
		"File 3 should NOT be included (no shared boundary)"
	);
}

#[test(tokio::test)]
async fn test_clean_cut_chain_expansion() {
	let env = TestEnv::new();
	...
	let table4 = env.create_test_table(4, file4_entries).await.unwrap();
	...
	let selected = Strategy::select_overlapping_ranges(source_level, test_sst_id(2)).unwrap();

	// Verify all three files in the chain are included
	assert!(selected.contains(&test_sst_id(1)), "File 1 should be included (chain expansion)");
	assert!(selected.contains(&test_sst_id(2)), "File 2 should be included (initially selected)");
	assert!(selected.contains(&test_sst_id(3)), "File 3 should be included (chain expansion)");
	// Verify file 4 is NOT included (no shared boundary)
	assert!(
		!selected.contains(&test_sst_id(4)),
		"File 4 should NOT be included (no shared boundary)"
	);
}

#[test(tokio::test)]
async fn test_clean_cut_no_expansion_needed() {
	let env = TestEnv::new();
	...
	assert!(selected.contains(&test_sst_id(1)), "File 1 should be the only selected file");
}

#[test(tokio::test)]
async fn test_clean_cut_integration_shared_boundary() {
	let env = TestEnv::new();
	...
	assert!(
		selected.contains(&test_sst_id(1)),
		"File 1 should be included (initially selected as largest)"
	);
	assert!(
		selected.contains(&test_sst_id(2)),
		"File 2 should be included (shares boundary key 'foo' with file 1)"
...
#[test(tokio::test)]
async fn test_clean_cut_integration_chain_expansion() {
	let env = TestEnv::new();
	...
	assert!(
		selected.contains(&test_sst_id(2)),
		"File 2 should be included (initially selected as largest)"
	);
	assert!(selected.contains(&test_sst_id(3)), "File 3 should be included (chain expansion)");
	// Verify File 4 is NOT included (no shared boundary)
	assert!(
...
#[test(tokio::test)]
async fn test_clean_cut_integration_with_oldest_seq_priority() {
	let env = TestEnv::new();
	...
	assert!(
		selected.contains(&test_sst_id(1)),
		"File 1 should be included (initially selected as oldest)"
	);
	assert!(
		selected.contains(&test_sst_id(2)),
		"File 2 should be included (shares boundary key 'foo' with file 1)"
...
#[test(tokio::test)]
async fn test_clean_cut_integration_no_expansion() {
	let env = TestEnv::new();
...
// ============================================================================
// Beat/Bar Compaction Tests
// ============================================================================

/// Helper to create a CompactionScheduler with test defaults.
/// Uses the same pattern as create_compaction_options() but wraps in a scheduler.
fn create_test_scheduler(
	...
	CompactionScheduler::new(
		...
		SnapshotTracker::new(),
		test_table_store(),
		Arc::new(ManifestUploader::new(tx)),
		0, // no fencing
	)
}

/// Helper to create a minimal empty manifest for scheduler tests.
fn create_empty_manifest(env: &TestEnv) -> Arc<RwLock<LevelManifest>> {
	let levels = Levels::new(env.options.level_count as usize, 10);
	let mut manifest = LevelManifest {
		manifest_dir: env.options.manifest_dir(),
		manifest_id: 0,
		levels,
...
// ── Group: Level Alternation Tests ──

#[test]
fn test_level_active_first_half_bar() {
	// First half (beats 0..16 with beats_per_bar=32): ODD target levels active
	let env = TestEnv::new_with_levels(7);
	let manifest = create_empty_manifest(&env);
	let scheduler = create_test_scheduler(Arc::clone(&env.options), manifest);

	assert_eq!(scheduler.get_beats_per_bar(), 32);
	assert_eq!(scheduler.get_half_bar(), 16);

	for beat in 0..16 {
		// Odd target levels should be active
		assert!(
			scheduler.is_level_active(1, beat),
			"Level 1 (odd) should be active at beat {beat}"
		);
		assert!(
			scheduler.is_level_active(3, beat),
			"Level 3 (odd) should be active at beat {beat}"
		);
		assert!(
			scheduler.is_level_active(5, beat),
			"Level 5 (odd) should be active at beat {beat}"
		);

		// Even target levels should be inactive
		assert!(
			!scheduler.is_level_active(0, beat),
			"Level 0 (even) should be inactive at beat {beat}"
		);
		assert!(
			!scheduler.is_level_active(2, beat),
			"Level 2 (even) should be inactive at beat {beat}"
		);
		assert!(
			!scheduler.is_level_active(4, beat),
			"Level 4 (even) should be inactive at beat {beat}"
		);
		assert!(
			!scheduler.is_level_active(6, beat),
			"Level 6 (even) should be inactive at beat {beat}"
		);
	}
}

#[test]
fn test_level_active_second_half_bar() {
	// Second half (beats 16..32): EVEN target levels active
	let env = TestEnv::new_with_levels(7);
	let manifest = create_empty_manifest(&env);
	let scheduler = create_test_scheduler(Arc::clone(&env.options), manifest);

	for beat in 16..32 {
		// Even target levels should be active
		assert!(
			scheduler.is_level_active(0, beat),
			"Level 0 (even) should be active at beat {beat}"
		);
		assert!(
			scheduler.is_level_active(2, beat),
			"Level 2 (even) should be active at beat {beat}"
		);
		assert!(
			scheduler.is_level_active(4, beat),
			"Level 4 (even) should be active at beat {beat}"
		);
		assert!(
			scheduler.is_level_active(6, beat),
			"Level 6 (even) should be active at beat {beat}"
		);

		// Odd target levels should be inactive
		assert!(
			!scheduler.is_level_active(1, beat),
			"Level 1 (odd) should be inactive at beat {beat}"
		);
		assert!(
			!scheduler.is_level_active(3, beat),
			"Level 3 (odd) should be inactive at beat {beat}"
		);
		assert!(
			!scheduler.is_level_active(5, beat),
			"Level 5 (odd) should be inactive at beat {beat}"
		);
	}
}

#[test]
fn test_level_active_wraps_across_bars() {
	// level_active() receives already-normalized beats (beat % beats_per_bar) from
	// compact_beat(). Verify that the normalization in compact_beat() means bar
	// boundaries work correctly by checking beats 0 and half_bar directly.
	let env = TestEnv::new_with_levels(4);
	let manifest = create_empty_manifest(&env);
	let scheduler = create_test_scheduler(Arc::clone(&env.options), manifest);

	let half_bar = scheduler.get_half_bar();

	// Beat 0 (start of any bar): first half, odd active
	assert!(scheduler.is_level_active(1, 0));
	assert!(!scheduler.is_level_active(2, 0));

	// Beat half_bar (start of second half): even active
	assert!(!scheduler.is_level_active(1, half_bar));
	assert!(scheduler.is_level_active(2, half_bar));

	// Last beat of first half (half_bar - 1): still first half, odd active
	assert!(scheduler.is_level_active(1, half_bar - 1));
	assert!(!scheduler.is_level_active(2, half_bar - 1));

	// Last beat of second half (beats_per_bar - 1): still second half, even active
	let beats_per_bar = scheduler.get_beats_per_bar();
	assert!(!scheduler.is_level_active(1, beats_per_bar - 1));
	assert!(scheduler.is_level_active(2, beats_per_bar - 1));
}

#[test]
fn test_level_active_custom_beats_per_bar() {
	// With beats_per_bar=4 (half_bar=2): beats 0,1 → odd active; beats 2,3 → even active
	let env = TestEnv::new_with_levels(4);
	let mut opts = (*env.options).clone();
	opts.compaction_beats_per_bar = 4;
	let opts = Arc::new(opts);
	let manifest = create_empty_manifest(&env);
	let scheduler = create_test_scheduler(opts, manifest);

	assert_eq!(scheduler.get_beats_per_bar(), 4);
	assert_eq!(scheduler.get_half_bar(), 2);

	// First half: beats 0, 1
	assert!(scheduler.is_level_active(1, 0));
	assert!(!scheduler.is_level_active(2, 0));
	assert!(scheduler.is_level_active(1, 1));
	assert!(!scheduler.is_level_active(2, 1));

	// Second half: beats 2, 3
	assert!(!scheduler.is_level_active(1, 2));
	assert!(scheduler.is_level_active(2, 2));
	assert!(!scheduler.is_level_active(1, 3));
	assert!(scheduler.is_level_active(2, 3));
}

// ── Group: Beat Sequencing Tests ──
// Tests the compact_beat() lifecycle through full bars.

#[test(tokio::test)]
async fn test_beat_sequencing_empty_levels() {
	// Running a full bar on empty manifest should complete without errors
	let env = TestEnv::new_with_levels(4);
	let manifest = create_empty_manifest(&env);
	let mut scheduler = create_test_scheduler(Arc::clone(&env.options), manifest);

	assert_eq!(scheduler.current_op(), 0);

	// Run a full bar (32 beats)
	for _ in 0..32 {
		scheduler.compact_beat().await.unwrap();
	}

	assert_eq!(scheduler.current_op(), 32);
	assert_eq!(scheduler.active_compaction_count(), 0);
}

#[test(tokio::test)]
async fn test_beat_sequencing_op_counter() {
	// Verify op counter increments by 1 each beat
	let env = TestEnv::new_with_levels(4);
	let manifest = create_empty_manifest(&env);
	let mut scheduler = create_test_scheduler(Arc::clone(&env.options), manifest);

	for i in 0..100 {
		assert_eq!(scheduler.current_op(), i);
		scheduler.compact_beat().await.unwrap();
	}
	assert_eq!(scheduler.current_op(), 100);
}

#[test(tokio::test)]
async fn test_beat_sequencing_half_bar_boundaries() {
	// With enough L0 tables to trigger compaction, verify compactions
	// are created at half-bar start and committed at half-bar end.
	let env = TestEnv::new_with_levels(4);

	// Create 5 L0 tables (default level0_max_files = 4, so this triggers compaction)
	let level_tables = vec![
		// L0: 5 tables with non-overlapping key ranges
		vec![
			(1, 100, 0, 19),
			(2, 200, 20, 39),
			(3, 300, 40, 59),
	...
	let mut opts = (*env.options).clone();
	opts.compaction_beats_per_bar = 8; // half_bar = 4
	opts.level0_max_files = 4;
	let opts = Arc::new(opts);

	let mut scheduler = create_test_scheduler(opts, Arc::clone(&manifest));

	// Beat 0: half-bar start — compactions should be created for active odd levels
	// L0→L1 has target_level=1 (odd), active in first half
	scheduler.compact_beat().await.unwrap();
	// After beat 0, we're past half-bar start; active compactions may exist
	// (depends on whether L0 trigger is met for the targeted strategy)

	// Run remaining beats of first half-bar
	for _ in 1..4 {
		scheduler.compact_beat().await.unwrap();
	}
	// After beat 3 (last beat of first half): complete_half_bar was called
	assert_eq!(
		scheduler.active_compaction_count(),
		0,
		"All compactions should be committed at half-bar end"
	);

	// Second half-bar (beats 4-7): even levels active
	for _ in 4..8 {
		scheduler.compact_beat().await.unwrap();
	}
	assert_eq!(scheduler.current_op(), 8);
	assert_eq!(scheduler.active_compaction_count(), 0);

	// Verify L0 tables were compacted — should have fewer tables in L0 now
	let levels = manifest.read().unwrap();
	let l0_count = levels.levels.get_levels()[0].tables.len();
	// If compaction happened, L0 should have fewer tables
	// (may still have tables if strategy decided differently, but at least no error)
	assert!(l0_count <= 5, "L0 should have at most the original 5 tables, got {l0_count}");
}

#[test(tokio::test)]
async fn test_beat_sequencing_multiple_bars() {
	// Run 2 full bars, verify op counter and no errors
	let env = TestEnv::new_with_levels(4);
	let manifest = create_empty_manifest(&env);
	let mut opts = (*env.options).clone();
	opts.compaction_beats_per_bar = 8;
	let opts = Arc::new(opts);
	let mut scheduler = create_test_scheduler(opts, manifest);
	...
	assert_eq!(scheduler.current_op(), 16);
	assert_eq!(scheduler.active_compaction_count(), 0);
}

// ── Group: End-to-End Beat Compaction Correctness ──
// Verifies data integrity after beat-paced compaction.

#[test(tokio::test)]
async fn test_beat_compaction_preserves_all_keys() {
	// Create tables with known data, run beat compaction, verify all keys survive
	let env = TestEnv::new_with_levels(4);
	...
	for i in 0..100u64 {
		...
		expected_keys.insert((key, create_inline_value(&value)));
	}

	// Create 5 L0 tables with 20 keys each
	let level_tables = vec![
		vec![
			(1, 100, 0, 19),
			(2, 200, 20, 39),
	...
	let mut opts = (*env.options).clone();
	opts.compaction_beats_per_bar = 8;
	opts.level0_max_files = 4; // trigger compaction with 5 L0 tables
	let opts = Arc::new(opts);

	let mut scheduler = create_test_scheduler(Arc::clone(&opts), Arc::clone(&manifest));
	...
	verify_keys_after_compaction(&manifest, &expected_keys).await;
}

#[test(tokio::test)]
async fn test_beat_compaction_handles_tombstones() {
	// Mix of Set + Delete entries — tombstones at bottom level should be dropped
	let env = TestEnv::new_with_levels(2); // 2 levels: L0, L1 (L1 is bottom)

	// Create L0 table with some keys
	let mut entries = Vec::new();
	for i in 0..20u64 {
		let user_key = format!("key-{i:010}").into_bytes();
		let key = InternalKey::new(user_key, 100 + i, InternalKeyKind::Set, 0);
		let value = format!("value-{i}").into_bytes();
		entries.push((key, create_inline_value(&value)));
	}

	// Create another L0 table with tombstones for keys 10-19
	let mut tombstone_entries = Vec::new();
	for i in 10..20u64 {
		let user_key = format!("key-{i:010}").into_bytes();
		let key = InternalKey::new(user_key, 200 + i, InternalKeyKind::Delete, 0);
		tombstone_entries.push((key, Vec::new()));
	}

	let table1 = env.create_test_table(1, entries).await.unwrap();
	let table2 = env.create_test_table(2, tombstone_entries).await.unwrap();
	...
	// After compaction to L1 (bottom level), tombstones should be dropped.
	// Only keys 0-9 should survive.
	let expected_surviving_keys: HashSet<_> = (0..10u64)
		.map(|i| {
			let key = format!("key-{i:010}").into_bytes();
			let value = format!("value-{i}").into_bytes();
			(key, create_inline_value(&value))
		})
		.collect();

	// Collect tables under lock, then drop lock before async iteration
	let tables: Vec<Arc<Table>> = {
		let levels = manifest.read().unwrap();
		levels.levels.get_levels().iter().flat_map(|level| level.tables.iter().cloned()).collect()
	};

	// Collect all live keys across all levels
	let mut all_keys = HashMap::new();
	for table in &tables {
		let mut iter = table.iter(None).unwrap();
		iter.seek_first().await.unwrap();
		while iter.valid() {
			let ikey = iter.key().to_owned();
			if ikey.kind() != InternalKeyKind::Delete {
				let key = ikey.user_key.clone();
				let value = iter.value_encoded().unwrap().to_vec();
				all_keys.insert(key, value);
			}
			iter.next().await.unwrap();
	...
	// Verify surviving keys match expectation
	for (expected_key, expected_value) in &expected_surviving_keys {
		let actual_value = all_keys.get(expected_key);
		assert!(actual_value.is_some(), "Key {expected_key:?} should survive compaction");
		assert_eq!(
			actual_value.unwrap(),
			expected_value,
...
#[test(tokio::test)]
async fn test_beat_compaction_overlapping_keys() {
	// Multiple L0 tables with same user keys at different sequence numbers.
	// Latest version should survive after compaction.
	let env = TestEnv::new_with_levels(4);

	// Table 1: keys 0-9 at seq 100
	let entries1: Vec<_> = (0..10u64)
		.map(|i| {
			let user_key = format!("key-{i:010}").into_bytes();
			let key = InternalKey::new(user_key, 100 + i, InternalKeyKind::Set, 0);
			let value = format!("old-value-{i}").into_bytes();
			(key, create_inline_value(&value))
		})
		.collect();

	// Table 2: keys 0-9 at seq 200 (newer — should win)
	let entries2: Vec<_> = (0..10u64)
		.map(|i| {
			let user_key = format!("key-{i:010}").into_bytes();
			let key = InternalKey::new(user_key, 200 + i, InternalKeyKind::Set, 0);
			let value = format!("new-value-{i}").into_bytes();
			(key, create_inline_value(&value))
		})
		.collect();

	// Table 3: keys 10-19 at seq 300
	let entries3: Vec<_> = (10..20u64)
		.map(|i| {
			let user_key = format!("key-{i:010}").into_bytes();
			let key = InternalKey::new(user_key, 300 + i, InternalKeyKind::Set, 0);
			let value = format!("unique-value-{i}").into_bytes();
			(key, create_inline_value(&value))
		})
		.collect();

	// Additional padding tables to trigger compaction (need >= level0_max_files)
	let entries4 = create_test_entries(20, 29, 400, "pad1");
	let entries5 = create_test_entries(30, 39, 500, "pad2");

	let t1 = env.create_test_table(1, entries1).await.unwrap();
	let t2 = env.create_test_table(2, entries2).await.unwrap();
	let t3 = env.create_test_table(3, entries3).await.unwrap();
	let t4 = env.create_test_table(4, entries4).await.unwrap();
	let t5 = env.create_test_table(5, entries5).await.unwrap();

	let mut levels = Levels::new(4, 10);
	for t in [t1, t2, t3, t4, t5] {
		Arc::make_mut(&mut levels.get_levels_mut()[0]).insert(t);
	}
	...
	// Verify latest values won for overlapping keys
	let mut all_keys = HashMap::new();
	for table in &tables {
		let mut iter = table.iter(None).unwrap();
		iter.seek_first().await.unwrap();
		while iter.valid() {
			...
			let key = ikey.user_key.clone();
			let value = iter.value_encoded().unwrap().to_vec();
			// In case of duplicate user keys across levels, prefer higher seqnum
			all_keys
				.entry(key)
				.and_modify(|(existing_seq, existing_val): &mut (u64, Vec<u8>)| {
					if ikey.seq_num() > *existing_seq {
						*existing_seq = ikey.seq_num();
						*existing_val = value.clone();
					}
				})
				.or_insert((ikey.seq_num(), value));
			iter.next().await.unwrap();
		}
	}

	// Keys 0-9 should have "new-value-*" (from table 2, seq 200+)
	for i in 0..10u64 {
		let key = format!("key-{i:010}").into_bytes();
		let expected_value = create_inline_value(format!("new-value-{i}").as_bytes());
		let (_, actual_value) = all_keys.get(&key).unwrap_or_else(|| panic!("Missing key {i}"));
		assert_eq!(
			actual_value, &expected_value,
			"Key {i} should have new-value (latest seq wins)"
		);
	}
}
...
// ── Group: Fuzz Test ──
#[test(tokio::test)]
async fn test_beat_compaction_fuzz() {
	// Use the Tree API so compact_beat() fires naturally on each commit.
	// Verify every get() against an in-memory BTreeMap model.
	use std::collections::BTreeMap;

	let temp_dir = tempfile::TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		max_memtable_size: 4 * 1024, // 4KB — triggers frequent flushes
		compaction_beats_per_bar: 8,
		level_count: 4,
		level0_max_files: 4,
		level_multiplier: 4.0,
		max_bytes_for_level: 16 * 1024, // 16KB
		..Default::default()
	});

	let tree = crate::Tree::new(opts).await.unwrap();
	let mut model: BTreeMap<Vec<u8>, Vec<u8>> = BTreeMap::new();
	let mut rng = fastrand::Rng::with_seed(42);

	let num_ops = 500;
	let key_space = 50; // Small key space to force overlaps
	...
	for op_idx in 0..num_ops {
		...
		if roll < 50 {
			// Put (50%)
			let key_num = rng.u32(0..key_space);
			let key = format!("fuzz-key-{key_num:05}").into_bytes();
			let value = format!("fuzz-val-{op_idx}-{key_num}").into_bytes();
			...
			match (tree_val, model_val) {
				(Some(tv), Some(mv)) => {
					assert_eq!(
						&tv,
						mv,
						"Mismatch at op {op_idx} for key fuzz-key-{key_num:05}: \
						 tree={:?}, model={:?}",
						String::from_utf8_lossy(&tv),
						String::from_utf8_lossy(mv)
					);
				}
				(None, None) => {} // Both agree key doesn't exist
				(Some(tv), None) => {
					panic!(
						"Op {op_idx}: tree has key fuzz-key-{key_num:05} = {:?}, but model says absent",
						String::from_utf8_lossy(&tv)
					);
				}
				(None, Some(mv)) => {
					panic!(
						"Op {op_idx}: model has key fuzz-key-{key_num:05} = {:?}, but tree says absent",
						String::from_utf8_lossy(mv)
					);
				}
			}
	...
	// Final verification: full model scan
	for (key, expected_value) in &model {
		let tree_val = tree.get(key).await.unwrap();
		assert_eq!(
			tree_val.as_ref(),
			Some(expected_value),
			"Final check: mismatch for key {:?}",
...
#[test(tokio::test)]
async fn test_beat_compaction_fuzz_small() {
	// Smaller deterministic fuzz — fast regression test
	use std::collections::BTreeMap;

	let temp_dir = tempfile::TempDir::new().unwrap();
	let opts = Arc::new(Options {
		path: temp_dir.path().to_path_buf(),
		max_memtable_size: 4 * 1024,
		compaction_beats_per_bar: 4,
		level_count: 3,
		level0_max_files: 2,
		level_multiplier: 2.0,
		max_bytes_for_level: 8 * 1024,
		..Default::default()
	});
	...
	for op_idx in 0..50 {
		let roll = rng.u32(0..100);
		let key_num = rng.u32(0..20);
		let key = format!("k-{key_num:03}").into_bytes();

		if roll < 60 {
			let value = format!("v-{op_idx}").into_bytes();
			tree.set(&key, &value).await.unwrap();
			model.insert(key, value);
		} else if roll < 75 {
			tree.delete(&key).await.unwrap();
			...
			let model_val = model.get(&key);
			match (tree_val, model_val) {
				(Some(tv), Some(mv)) => assert_eq!(&tv, mv, "Mismatch at op {op_idx}"),
				(None, None) => {}
				(Some(_), None) => panic!("Op {op_idx}: tree has key, model doesn't"),
				(None, Some(_)) => panic!("Op {op_idx}: model has key, tree doesn't"),
			}
		}
	}
	...
	for (key, expected_value) in &model {
		let tree_val = tree.get(key).await.unwrap();
		assert_eq!(tree_val.as_ref(), Some(expected_value), "Final check mismatch");
	}
}
