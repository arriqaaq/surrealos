...
async fn test_range_query() {
	...
	range_iter.seek_first().await.unwrap();
	...
	while range_iter.valid() {
		...
		if !range_iter.next().await.unwrap_or(false) {
			break;
		}
	}
...
#[test(tokio::test)]
async fn test_range_query_with_sequence_numbers() {
...
#[test(tokio::test)]
async fn test_excluded_bound_skips_all_versions_of_key() {
	...
	while iter.valid() {
		...
		if key.user_key != b"b" {
			...
			while iter.next().await.unwrap_or(false) && iter.valid() {
				let key = iter.key().to_owned();
				let value_bytes = iter.value_encoded();
				let value = value_bytes.unwrap().to_vec();
				range_entries.push((key, value));
...
#[test(tokio::test)]
async fn test_excluded_bound_first_skips_all_versions() {
	let (memtable, _) = create_test_memtable(vec![
		(b"a".to_vec(), b"value-a".to_vec(), InternalKeyKind::Set, Some(10)),
	...
	// Iterate once
	let first = iter.seek_first().await.unwrap();
	assert!(first);
	let key = iter.key().to_owned();
	...
	while iter2.valid() {
		...
		if !iter2.next().await.unwrap_or(false) {
			break;
		}
	}

	let first = iter2.seek_first().await.unwrap();
	assert!(first);
	let key = iter2.key().to_owned();
...
// =============================================================================
// Memtable Deduplication Tests
// =============================================================================

/// Test: multiple writes to same key across batches, verify all versions stored
/// and latest version wins via get().
#[test(tokio::test)]
async fn test_memtable_dedup_across_sequences() {
	let memtable = MemTable::default();
	...
	let mut iter = memtable.iter();
	assert!(iter.seek_first().await.unwrap());

	let mut versions: Vec<(u64, InternalKeyKind)> = Vec::new();
	loop {
		let key = iter.key().to_owned();
		if key.user_key != b"k1" {
			break;
		}
		versions.push((key.seq_num(), key.kind()));
		if !iter.next().await.unwrap() {
			break;
		}
	}

	assert_eq!(versions.len(), 4, "Should have 4 versions of k1");
	// Descending seq order
	assert_eq!(versions[0], (4, InternalKeyKind::Set));
	assert_eq!(versions[1], (3, InternalKeyKind::Delete));
	assert_eq!(versions[2], (2, InternalKeyKind::Set));
	assert_eq!(versions[3], (1, InternalKeyKind::Set));

	// Verify via get: should return latest (seq=4, v3)
	let result = memtable.get(b"k1", None).unwrap();
	assert_eq!(result.0.seq_num(), 4);
	assert_eq!(result.0.kind(), InternalKeyKind::Set);
	assert_value(&result.1, b"v3");

	// Verify get at specific sequence: get at seq=2 should return v2
	let result = memtable.get(b"k1", Some(2)).unwrap();
	assert_eq!(result.0.seq_num(), 2);
	assert_value(&result.1, b"v2");

	// Get at seq=3 should return delete tombstone
	let result = memtable.get(b"k1", Some(3)).unwrap();
	assert_eq!(result.0.seq_num(), 3);
	assert_eq!(result.0.kind(), InternalKeyKind::Delete);
}

/// Test: tombstone interaction at Store level — delete shadows value, resurrection works.
#[test(tokio::test)]
async fn test_memtable_tombstone_interaction_via_store() {
	let temp_dir = tempfile::tempdir().unwrap();
	let path = temp_dir.path().to_path_buf();

	let tree = crate::Tree::new(Arc::new(crate::Options {
		path,
		..Default::default()
	}))
	.await
	...
	// Set k1=v1
	tree.set(b"k1", b"v1").await.unwrap();
	let val = tree.get(b"k1").await.unwrap();
	assert_eq!(val.as_deref(), Some(b"v1".as_slice()), "k1 should be v1");

	// Delete k1 (tombstone)
	tree.delete(b"k1").await.unwrap();
	let val = tree.get(b"k1").await.unwrap();
	assert_eq!(val, None, "k1 should be None after delete");

	// Resurrect k1=v2
	tree.set(b"k1", b"v2").await.unwrap();
	let val = tree.get(b"k1").await.unwrap();
	assert_eq!(val.as_deref(), Some(b"v2".as_slice()), "k1 should be v2 after resurrection");

	// Verify other keys are unaffected
	tree.set(b"k2", b"v_k2").await.unwrap();
	let val = tree.get(b"k2").await.unwrap();
	assert_eq!(val.as_deref(), Some(b"v_k2".as_slice()));

	// Delete k2 then verify k1 still exists
	tree.delete(b"k2").await.unwrap();
	let val = tree.get(b"k1").await.unwrap();
	assert_eq!(val.as_deref(), Some(b"v2".as_slice()), "k1 should still be v2");
	let val = tree.get(b"k2").await.unwrap();
	assert_eq!(val, None, "k2 should be None");
}
