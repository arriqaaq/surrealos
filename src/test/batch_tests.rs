...
fn test_batch_encode() {
	let mut batch = Batch::new();
	batch.set_at(b"key1", b"value1", 0).unwrap();
...
fn test_batch_multiple_operations() {
	let mut batch = Batch::new_with_seq(1);
	batch.set_at(b"key1", b"value1", 1).unwrap();
	batch.delete_at(b"key2", 2).unwrap();
	batch.set_at(b"key3", b"value3", 3).unwrap();
...
fn test_batch_empty_key_and_value() {
	let mut batch = Batch::new_with_seq(1);
	batch.set_at(b"", b"", 0).unwrap();
	batch.delete_at(b"", 0).unwrap();
...
fn test_batch_unicode_keys_and_values() {
	let mut batch = Batch::new_with_seq(1);
	batch.set_at("🔑".as_bytes(), "🗝️".as_bytes(), 1).unwrap();
	batch.set_at("こんにちは".as_bytes(), "世界".as_bytes(), 2).unwrap();
...
fn test_batch_sequence_numbers() {
	let mut batch = Batch::new_with_seq(100);
	batch.set_at(b"key1", b"value1", 1).unwrap();
	batch.set_at(b"key2", b"value2", 2).unwrap();
...
fn test_batch_large_number_of_records() {
	...
	for i in 0..NUM_RECORDS {
		...
		if i % 2 == 0 {
			batch.set_at(key.as_bytes(), value.as_bytes(), i as u64).unwrap();
		} else {
			batch.delete_at(key.as_bytes(), i as u64).unwrap();
		}
	}
...
fn test_batch_version() {
	...
	batch_with_data.set_at(b"key", b"value", 1).unwrap();
