...
use crate::manifest::{find_latest_local_manifest, validate_wal_log_number};
use crate::paths::StorePaths;
use crate::sstable::SstId;
use crate::test::{collect_transaction_all, test_sst_id};
...
/// Creates a CloudStore from Options, matching how CoreInner creates one.
fn create_table_store(opts: &Options) -> Arc<CloudStore> {
	Arc::new(CloudStore::new(
		Arc::clone(&opts.object_store),
		StorePaths::new(opts.object_store_root.as_str()),
		None,
	))
}
...
/// Creates test options with a shared object store, allowing customization of
/// specific fields. Used for tests that reopen the tree and need SSTs to persist.
fn create_test_options_with_store(
	path: PathBuf,
...
/// Creates test options with small memtable and a shared object store
fn create_small_memtable_options_with_store(
	path: PathBuf,
	store: Arc<dyn object_store::ObjectStore>,
) -> Arc<Options> {
	create_test_options_with_store(path, store, |opts| {
		opts.max_memtable_size = 64 * 1024; // 64KB
	})
}

/// Count local SST files in the sst directory.
fn count_local_sst_files(path: &Path) -> usize {
...
async fn test_memtable_flush() {
	...
	// Verify local SST files exist after flush
	assert!(count_local_sst_files(&path) > 0, "Expected local SST files after flush");
...
async fn test_memtable_flush_with_multiple_keys_and_updates() {
	...
	// Verify local SST files exist after flush
	assert!(
		count_local_sst_files(&path) > 0,
		"Expected local SST files after flush with multiple keys"
	);
...
async fn test_persistence() {
	...
	// Step 1: Insert data and close the store
	{
		...
		// Verify local SST files exist before closing
		let sst_count_before = count_local_sst_files(&path);
		assert!(sst_count_before > 0, "Expected local SST files before closing");

		// Tree will be dropped here, closing the store
		tree.close().await.unwrap();
	}

	// Verify local SST files persist after close
	assert!(count_local_sst_files(&path) > 0, "Expected local SST files to persist after close");
	...
	// Step 2: Reopen the store and verify data
	{
		...
		// Verify local SST files still present after reopen
		assert!(count_local_sst_files(&path) > 0, "Expected local SST files after reopening");
...
async fn test_flush_on_close_creates_sst() {
	...
	let opts = create_test_options_with_store(path.clone(), Arc::clone(&store), |opts| {
		opts.max_memtable_size = 10 * 1024 * 1024; // Large to prevent auto-flush
		opts.flush_on_close = true;
	});
...
async fn test_valid_ssts_not_deleted_during_cleanup() {
	...
	let store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::local::LocalFileSystem::new_with_prefix(path.clone()).unwrap());
	let opts = create_small_memtable_options_with_store(path.clone(), Arc::clone(&store));
...
async fn test_recovery_detects_corrupt_log_number() {
	...
	// Phase 2: Corrupt the manifest by setting log_number to a very high value
	{
		...
		let (_id, manifest_path) =
			find_latest_local_manifest(&manifest_dir).expect("Should find manifest");
	...
	{
		let opts = create_test_options_with_store(path.clone(), Arc::clone(&store), |_| {});
...
async fn test_recovery_detects_corrupt_log_number_multiple_wals() {
	...
	// Phase 3: Corrupt the manifest file
	{
		...
		corrupted.extend_from_slice(&data[2..10]); // manifest_id (u64)
...
#[test(tokio::test)]
async fn test_branch_creation_and_isolation() {
	// Shared object store — branches share SSTs here
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());

	// Parent store
	let parent_dir = create_temp_directory();
	let parent_opts = create_test_options_with_store(
		parent_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
		|_| {},
	);
	let parent = Tree::new(Arc::clone(&parent_opts)).await.unwrap();

	// Insert data into parent and flush to SSTs
	let mut batch = parent.new_batch();
	batch.set(b"key1", b"parent_value1").unwrap();
	batch.set(b"key2", b"parent_value2").unwrap();
	parent.apply(batch, false).await.unwrap();
	parent.flush().await.unwrap();

	// Create branch
	let branch_info = parent.create_branch("dev").await.unwrap();
	assert_eq!(branch_info.name, "dev");

	// Open branch store (separate local dir, same object store)
	let branch_dir = create_temp_directory();
	let branch_opts = create_test_options_with_store(
		branch_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
		|opts| {
			opts.branch_name = Some("dev".to_string());
		},
	);
	let branch = Tree::new(Arc::clone(&branch_opts)).await.unwrap();

	// Branch should see parent's data
	let v = branch.get(b"key1").await.unwrap().unwrap();
	assert_eq!(v, b"parent_value1");
	let v = branch.get(b"key2").await.unwrap().unwrap();
	assert_eq!(v, b"parent_value2");

	// Write new data to parent
	let mut batch = parent.new_batch();
	batch.set(b"key3", b"parent_only").unwrap();
	parent.apply(batch, false).await.unwrap();
	parent.flush().await.unwrap();

	// Branch should NOT see parent's new data
	assert!(branch.get(b"key3").await.unwrap().is_none());

	// Write new data to branch
	let mut batch = branch.new_batch();
	batch.set(b"key4", b"branch_only").unwrap();
	branch.apply(batch, false).await.unwrap();
	branch.flush().await.unwrap();

	// Parent should NOT see branch's data
	assert!(parent.get(b"key4").await.unwrap().is_none());

	// Branch should see its own data
	let v = branch.get(b"key4").await.unwrap().unwrap();
	assert_eq!(v, b"branch_only");

	// Both should still see original shared data
	let v = parent.get(b"key1").await.unwrap().unwrap();
	assert_eq!(v, b"parent_value1");
	let v = branch.get(b"key1").await.unwrap().unwrap();
	assert_eq!(v, b"parent_value1");
}

/// Adapted from RocksDB-Cloud TrueClone (lines 808-813).
/// Branch can overwrite a key inherited from parent.
#[test(tokio::test)]
async fn test_branch_overwrite_key() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());
	...
	// Parent writes "Hello" = "World"
	let mut batch = parent.new_batch();
	batch.set(b"Hello", b"World").unwrap();
	parent.apply(batch, false).await.unwrap();
	parent.flush().await.unwrap();

	parent.create_branch("dev").await.unwrap();

	// Open branch and overwrite "Hello"
	let branch_dir = create_temp_directory();
	let branch_opts = create_test_options_with_store(
		branch_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
	...
	let mut batch = branch.new_batch();
	batch.set(b"Hello", b"Clone1").unwrap();
	branch.apply(batch, false).await.unwrap();
	branch.flush().await.unwrap();

	// Branch sees overwritten value
	let v = branch.get(b"Hello").await.unwrap().unwrap();
	assert_eq!(v, b"Clone1");

	// Parent still sees original value
	let v = parent.get(b"Hello").await.unwrap().unwrap();
	assert_eq!(v, b"World");
}

/// Adapted from RocksDB-Cloud TrueClone (lines 815-828).
/// Branch data persists in cloud after close+reopen with different local dir.
#[test(tokio::test)]
async fn test_branch_reopen_persistence() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());
	...
	// Open branch, write data, close
	{
		...
		let mut batch = branch.new_batch();
		batch.set(b"branch_key", b"branch_value").unwrap();
		branch.apply(batch, false).await.unwrap();
		branch.flush().await.unwrap();
		branch.close().await.unwrap();
	}

	// Reopen branch with DIFFERENT local dir — data should persist from cloud
	{
		let branch_dir2 = create_temp_directory();
		let branch_opts2 = create_test_options_with_store(
			branch_dir2.path().to_path_buf(),
			Arc::clone(&shared_store),
			|opts| {
				opts.branch_name = Some("dev".to_string());
			},
		);
		let branch2 = Tree::new(Arc::clone(&branch_opts2)).await.unwrap();

		// Should see both inherited and branch-written data
		let v = branch2.get(b"key1").await.unwrap().unwrap();
		assert_eq!(v, b"value1");
		let v = branch2.get(b"branch_key").await.unwrap().unwrap();
		assert_eq!(v, b"branch_value");
	}
}

/// Adapted from RocksDB-Cloud TrueClone (lines 845-861) + Newdb (lines 694-716).
/// Multiple branches from same parent: each sees parent's data, not sibling's writes.
#[test(tokio::test)]
async fn test_branch_fan_out() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());
	...
	let mut batch = parent.new_batch();
	batch.set(b"key1", b"original").unwrap();
	parent.apply(batch, false).await.unwrap();
	parent.flush().await.unwrap();

	// Create dev1, open it, overwrite key1
	parent.create_branch("dev1").await.unwrap();
	let dev1_dir = create_temp_directory();
	let dev1_opts = create_test_options_with_store(
		dev1_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
		|opts| {
			opts.branch_name = Some("dev1".to_string());
		},
	);
	let dev1 = Tree::new(Arc::clone(&dev1_opts)).await.unwrap();
	let mut batch = dev1.new_batch();
	batch.set(b"key1", b"dev1_value").unwrap();
	dev1.apply(batch, false).await.unwrap();
	dev1.flush().await.unwrap();

	// Create dev2 from parent (not from dev1)
	parent.create_branch("dev2").await.unwrap();
	let dev2_dir = create_temp_directory();
	let dev2_opts = create_test_options_with_store(
		dev2_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
		|opts| {
			opts.branch_name = Some("dev2".to_string());
		},
	);
	let dev2 = Tree::new(Arc::clone(&dev2_opts)).await.unwrap();

	// dev2 should see original value, not dev1's overwrite
	let v = dev2.get(b"key1").await.unwrap().unwrap();
	assert_eq!(v, b"original");

	// dev1 should still see its own value
	let v = dev1.get(b"key1").await.unwrap().unwrap();
	assert_eq!(v, b"dev1_value");

	// Parent should see original
	let v = parent.get(b"key1").await.unwrap().unwrap();
	assert_eq!(v, b"original");
}

/// Adapted from RocksDB-Cloud DbidRegistry (lines 887-902).
/// After branch creation, the registry tracks the branch with correct metadata.
#[test(tokio::test)]
async fn test_branch_registry_tracking() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());
	...
	let info = parent.create_branch("dev").await.unwrap();

	// Load registry from object store and verify
	let root_path: object_store::path::Path = "".into();
	let registry = crate::branch::load_registry(shared_store.as_ref(), &root_path).await.unwrap();

	assert!(registry.contains("dev"));
	let branch_info = registry.get("dev").unwrap();
	assert_eq!(branch_info.status, crate::branch::BranchStatus::Active);
	assert_eq!(branch_info.parent_branch, None); // parent has no branch_name
	assert_eq!(branch_info.source_manifest_id, info.source_manifest_id);
}

/// Adapted from RocksDB-Cloud TrueClone FindObsoleteFiles (lines 863-875).
/// After branch creation, collect_live_ssts includes SSTs from all branches.
#[test(tokio::test)]
async fn test_branch_gc_live_ssts() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());
	...
	// Create branch — branch manifest now references the same SSTs
	parent.create_branch("dev").await.unwrap();

	// Load registry and collect live SSTs
	let root_path: object_store::path::Path = "".into();
	let registry = crate::branch::load_registry(shared_store.as_ref(), &root_path).await.unwrap();

	let live = crate::gc::collect_live_ssts(&shared_store, "", &registry).await.unwrap();

	// Should have at least 1 SST referenced
	assert!(!live.is_empty(), "Live SST set should not be empty after branch creation");

	// Open branch and write more data (creates new SSTs in branch)
	let branch_dir = create_temp_directory();
	let branch_opts = create_test_options_with_store(
		branch_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
	...
	let mut batch = branch.new_batch();
	batch.set(b"key3", b"branch_only").unwrap();
	branch.apply(batch, false).await.unwrap();
	branch.flush().await.unwrap();

	// Re-collect live SSTs — should now include both parent's and branch's SSTs
	let registry2 = crate::branch::load_registry(shared_store.as_ref(), &root_path).await.unwrap();
	let live2 = crate::gc::collect_live_ssts(&shared_store, "", &registry2).await.unwrap();

	assert!(live2.len() >= live.len(), "Live set should grow after branch writes");
}

/// SurrealOS-specific: branches in "Creating" status protect parent SSTs from GC.
/// Simulates an interrupted create_branch() by creating a branch normally, then
/// resetting its registry status back to "Creating" — as if the crash happened
/// after registry write but before status update to Active.
#[test(tokio::test)]
async fn test_branch_creating_status_protects_ssts() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());
	...
	let mut batch = parent.new_batch();
	batch.set(b"protect_me", b"important").unwrap();
	parent.apply(batch, false).await.unwrap();
	parent.flush().await.unwrap();

	// Create branch normally (ensures parent manifest is uploaded to object store)
	let info = parent.create_branch("incomplete_branch").await.unwrap();

	// Simulate interrupted creation: reset status back to Creating in registry
	let root_path: object_store::path::Path = "".into();
	let mut registry =
		crate::branch::load_registry(shared_store.as_ref(), &root_path).await.unwrap();
	registry.update_status("incomplete_branch", crate::branch::BranchStatus::Creating).unwrap();
	crate::branch::save_registry(shared_store.as_ref(), &root_path, &registry).await.unwrap();

	// collect_live_ssts should include parent's SSTs because the Creating branch
	// references source_manifest_id
	let live = crate::gc::collect_live_ssts(&shared_store, "", &registry).await.unwrap();
	assert!(
		!live.is_empty(),
		"Creating branch should protect parent SSTs via source_manifest_id (source_manifest_id={})",
...
async fn test_branch_duplicate_name_rejected() {
	...
	let result = parent.create_branch("dev").await;
	assert!(result.is_err(), "Duplicate branch name should be rejected");
}

/// Adapted from RocksDB-Cloud KeepLocalFiles (lines 904-941).
/// Branch with local SST cache: data accessible after reopen, local files exist.
#[test(tokio::test)]
async fn test_branch_with_local_sst_cache() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());
	...
	let mut batch = parent.new_batch();
	batch.set(b"Hello2", b"World2").unwrap();
	parent.apply(batch, false).await.unwrap();
	parent.flush().await.unwrap();

	// Verify parent has local SST files
	assert!(count_local_sst_files(parent_dir.path()) >= 2, "Parent should have local SST files");

	parent.create_branch("dev").await.unwrap();

	// Open branch with local SST caching in a fresh dir
	let branch_dir = create_temp_directory();
	let branch_opts = create_test_options_with_store(
		branch_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
	...
	let v = branch.get(b"Hello").await.unwrap().unwrap();
	assert_eq!(v, b"World");
	let v = branch.get(b"Hello2").await.unwrap().unwrap();
	assert_eq!(v, b"World2");

	// Write to branch and flush — should create local SST files
	let mut batch = branch.new_batch();
	batch.set(b"branch_key", b"branch_val").unwrap();
	branch.apply(batch, false).await.unwrap();
	branch.flush().await.unwrap();

	assert!(
		count_local_sst_files(branch_dir.path()) >= 1,
		"Branch should have local SST files after flush"
	);
}
...
/// Fuzz test: random data + random range queries, compare with BTreeMap model.
/// Verifies range scan returns exactly the same results as the model.
#[test(tokio::test)]
async fn test_range_scan_correctness_fuzz() {
	let temp_dir = create_temp_directory();
	let path = temp_dir.path().to_path_buf();

	// Small memtable to force SST creation
	let tree = Tree::new(create_test_options(path, |opts| {
		opts.max_memtable_size = 4 * 1024; // 4KB
	}))
	.await
	.unwrap();
	...
	// Insert 200 random key-value pairs
	for _ in 0..200 {
		let key_len = rng.usize(4..=8);
		let key: Vec<u8> = (0..key_len).map(|_| rng.u8(b'a'..=b'z')).collect();
		let val_len = rng.usize(10..=20);
		let value: Vec<u8> = (0..val_len).map(|_| rng.u8(0..=255)).collect();

		tree.set(&key, &value).await.unwrap();
		model.insert(key, value);
	}

	// Fuzz 50 range queries
	for q in 0..50 {
		// Generate random range bounds
		let use_lower = rng.bool();
		let use_upper = rng.bool();

		let lower: Option<Vec<u8>> = if use_lower {
			let len = rng.usize(2..=6);
			Some((0..len).map(|_| rng.u8(b'a'..=b'z')).collect())
		} else {
			None
		};

		let upper: Option<Vec<u8>> = if use_upper {
			let u: Vec<u8> = if let Some(ref l) = lower {
				// Ensure upper > lower
				let mut v = l.clone();
				if let Some(last) = v.last_mut() {
					*last = (*last).saturating_add(rng.u8(1..=10));
				}
				v
			} else {
				let len = rng.usize(2..=6);
				(0..len).map(|_| rng.u8(b'a'..=b'z')).collect()
			};
			Some(u)
		} else {
			None
		...
		let mut iter = snap.range(lower.as_deref(), upper.as_deref()).unwrap();
		iter.seek_first().await.unwrap();

		let mut tree_results: Vec<(Vec<u8>, Vec<u8>)> = Vec::new();
		while iter.valid() {
			let key = iter.key().user_key().to_vec();
			let value = iter.value_encoded().unwrap().to_vec();
			tree_results.push((key, value));
			iter.next().await.unwrap();
		}

		// Model range scan (lower inclusive, upper exclusive — matches SurrealOS semantics)
		let model_results: Vec<(Vec<u8>, Vec<u8>)> = {
			use std::ops::Bound;
			let lo = match &lower {
			...
			model.range::<Vec<u8>, _>((lo, hi)).map(|(k, v)| (k.clone(), v.clone())).collect()
		};

		assert_eq!(
			tree_results.len(),
			model_results.len(),
			"query {q}: count mismatch for range {:?}..{:?}: tree={}, model={}",
			lower,
			upper,
			tree_results.len(),
			model_results.len()
		...
		for (i, ((tk, tv), (mk, mv))) in tree_results.iter().zip(model_results.iter()).enumerate() {
			assert_eq!(tk, mk, "query {q}: key mismatch at index {i}");
			assert_eq!(tv, mv, "query {q}: value mismatch at index {i} for key {:?}", tk);
		}
	}
}

/// Range scan boundary edge cases: empty range, full range, single key, etc.
#[test(tokio::test)]
async fn test_range_scan_boundaries() {
	let temp_dir = create_temp_directory();
	let path = temp_dir.path().to_path_buf();

	let tree = Tree::new(create_test_options(path, |_| {})).await.unwrap();

	// Insert known keys
	for key in &["aa", "bb", "cc", "dd", "ee"] {
		tree.set(key.as_bytes(), format!("val_{key}").as_bytes()).await.unwrap();
	}

	// Helper to count range results
	async fn scan_count(tree: &Tree, lower: Option<&[u8]>, upper: Option<&[u8]>) -> Vec<String> {
		let snap = tree.new_snapshot();
		let mut iter = snap.range(lower, upper).unwrap();
		iter.seek_first().await.unwrap();
	...
	// Empty range: lower == upper (exclusive upper)
	let r = scan_count(&tree, Some(b"aa"), Some(b"aa")).await;
	assert_eq!(r, Vec::<String>::new(), "range [aa, aa) should be empty");

	// Single key range
	let r = scan_count(&tree, Some(b"aa"), Some(b"ab")).await;
	assert_eq!(r, vec!["aa"], "range [aa, ab) should contain only aa");

	// Full range
	let r = scan_count(&tree, None, None).await;
	assert_eq!(r, vec!["aa", "bb", "cc", "dd", "ee"], "unbounded range should return all keys");

	// Middle range (exclusive upper)
	let r = scan_count(&tree, Some(b"bb"), Some(b"dd")).await;
	assert_eq!(r, vec!["bb", "cc"], "range [bb, dd) should return bb, cc");

	// Range past all keys
	let r = scan_count(&tree, Some(b"ff"), None).await;
	assert_eq!(r, Vec::<String>::new(), "range [ff, ..) should be empty");

	// Range before first key (exclusive upper)
	let r = scan_count(&tree, None, Some(b"aa")).await;
	assert_eq!(r, Vec::<String>::new(), "range (.., aa) should be empty");

	// Range up to but not including "bb"
	let r = scan_count(&tree, None, Some(b"bb")).await;
	assert_eq!(r, vec!["aa"], "range (.., bb) should return only aa");

	// Prefix-like range
	let r = scan_count(&tree, Some(b"a"), Some(b"b")).await;
	assert_eq!(r, vec!["aa"], "range [a, b) should return aa");
}

/// Range scan after compaction: verify data flows through L0→L1→L2 correctly
/// and range queries still return correct results.
#[test(tokio::test)]
async fn test_range_scan_after_compaction() {
	let temp_dir = create_temp_directory();
	let path = temp_dir.path().to_path_buf();

	// Very small memtable to force frequent flushes and compaction
	let tree = Tree::new(create_test_options(path, |opts| {
		opts.max_memtable_size = 4 * 1024; // 4KB
	}))
	.await
	...
	// Phase 1: Insert 300 entries in batches to trigger multiple flushes
	for i in 0..300u32 {
		let key = format!("key_{i:05}");
	...
	// Phase 2: Overwrite some keys to create newer versions
	for i in (0..300).step_by(3) {
		let key = format!("key_{i:05}");
		let value = format!("updated_{i:05}");
		tree.set(key.as_bytes(), value.as_bytes()).await.unwrap();
		model.insert(key.into_bytes(), value.into_bytes());
	}
	...
	let test_ranges: Vec<RangeSpec<'_>> = vec![
		...
		(Some(b"key_00225"), None),               // Last quarter
		(Some(b"key_00050"), Some(b"key_00051")), // Single key
		(Some(b"zzz"), None),                     // Empty (past all keys)
	];
	...
	for (lower, upper) in &test_ranges {
		...
		assert_eq!(
			tree_results.len(),
			model_results.len(),
			"range {:?}..{:?}: count mismatch: tree={}, model={}",
			lower.map(|b| String::from_utf8_lossy(b)),
			upper.map(|b| String::from_utf8_lossy(b)),
			tree_results.len(),
			model_results.len()
		);

		for (i, ((tk, tv), (mk, mv))) in tree_results.iter().zip(model_results.iter()).enumerate() {
			assert_eq!(tk, mk, "key mismatch at index {i}");
			assert_eq!(tv, mv, "value mismatch at index {i}");
		}
	}
}

/// Branch data isolation fuzz test: 50 key-value pairs on parent, fork to feature branch,
/// write additional data to feature, verify parent doesn't see feature data and feature
/// sees both inherited + new data. Uses model comparison for correctness.
/// Ported from TigerBeetle's Forest multi-tree isolation semantics.
#[test(tokio::test)]
async fn test_branch_data_isolation_fuzz() {
	let shared_store: Arc<dyn object_store::ObjectStore> =
		Arc::new(object_store::memory::InMemory::new());

	// Parent store with small memtable to force SSTs
	let parent_dir = create_temp_directory();
	let parent_opts = create_test_options_with_store(
		parent_dir.path().to_path_buf(),
		Arc::clone(&shared_store),
		|opts| {
			opts.max_memtable_size = 4 * 1024; // 4KB — forces multiple SSTs
		},
	);
	let parent = Tree::new(Arc::clone(&parent_opts)).await.unwrap();

	let mut rng = fastrand::Rng::with_seed(42);
	let mut model_main: std::collections::BTreeMap<Vec<u8>, Vec<u8>> =
		std::collections::BTreeMap::new();

	// Insert 50 key-value pairs into parent
	for i in 0..50u32 {
		let key = format!("key_{:04}", i).into_bytes();
		let val_len = rng.usize(10..=30);
		let value: Vec<u8> = (0..val_len).map(|_| rng.u8(0..=255)).collect();
		parent.set(&key, &value).await.unwrap();
		model_main.insert(key, value);
	}
	parent.flush().await.unwrap();

	// Create feature branch
	// Note: create_branch() returns the initial BranchInfo with Creating status;
	// the status is updated to Active in the registry after manifest write,
	// but the returned struct reflects the initial registration state.
	let branch_info = parent.create_branch("feature").await.unwrap();
	assert_eq!(branch_info.name, "feature");
	...
	let branch_opts = create_test_options_with_store(
		...
		Arc::clone(&shared_store),
		|opts| {
			opts.branch_name = Some("feature".to_string());
			opts.max_memtable_size = 4 * 1024;
		},
	);
	let feature = Tree::new(Arc::clone(&branch_opts)).await.unwrap();

	// Feature branch should see all inherited data
	for (key, value) in &model_main {
		let result = feature.get(key).await.unwrap();
		assert_eq!(
			result.as_deref(),
			Some(value.as_slice()),
			"Feature branch missing inherited key: {:?}",
			String::from_utf8_lossy(key)
		);
	}

	// Write 30 new keys to feature branch (some overlap with parent)
	let mut model_feature = model_main.clone();
	for i in 0..30u32 {
		let key = if i < 10 {
			// Overwrite 10 existing keys
			format!("key_{:04}", i).into_bytes()
		} else {
			// 20 new keys
			format!("branch_key_{:04}", i).into_bytes()
		};
		let val_len = rng.usize(10..=30);
		let value: Vec<u8> = (0..val_len).map(|_| rng.u8(0..=255)).collect();
		feature.set(&key, &value).await.unwrap();
		model_feature.insert(key, value);
	}
	feature.flush().await.unwrap();

	// Parent should NOT see any branch-only keys
	for i in 10..30u32 {
		let key = format!("branch_key_{:04}", i).into_bytes();
		assert!(
			parent.get(&key).await.unwrap().is_none(),
			"Parent should NOT see branch key: {:?}",
			String::from_utf8_lossy(&key)
		);
	}

	// Parent should still have original values for overwritten keys
	for i in 0..10u32 {
		let key = format!("key_{:04}", i).into_bytes();
	...
	// Feature should see its full model (inherited + new + overwrites)
	for (key, value) in &model_feature {
		let result = feature.get(key).await.unwrap();
		assert_eq!(
			result.as_deref(),
			Some(value.as_slice()),
			"Feature branch wrong value for key: {:?}",
			String::from_utf8_lossy(key)
		);
	}

	// Write more data to parent AFTER branch fork — feature should NOT see it
	let post_fork_key = b"post_fork_key";
	let post_fork_val = b"post_fork_val";
	parent.set(post_fork_key, post_fork_val).await.unwrap();
	parent.flush().await.unwrap();

	assert!(
		feature.get(post_fork_key).await.unwrap().is_none(),
		"Feature should NOT see post-fork parent data"
	);

	// Parent should see post-fork data
	assert_eq!(&parent.get(post_fork_key).await.unwrap().unwrap()[..], post_fork_val);

	// Full range scan on feature should match model
	let snap = feature.new_snapshot();
	...
	while iter.valid() {
		...
		let value = iter.value_encoded().unwrap().to_vec();
		assert!(
			model_feature.contains_key(&key),
			"Feature scan returned unexpected key: {:?}",
			String::from_utf8_lossy(&key)
		);
		assert_eq!(
			value,
	...
	assert_eq!(
		scan_count,
		model_feature.len(),
		"Feature range scan count mismatch: got {}, expected {}",
		scan_count,
		model_feature.len()
	);
}
