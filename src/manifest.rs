...
///
/// Uses a watch channel — the latest manifest always overwrites any pending
/// upload. Each manifest is a complete snapshot, so only the latest matters.
pub(crate) struct ManifestUploader {
	sender: tokio::sync::watch::Sender<Option<(u64, bytes::Bytes)>>,
}

impl ManifestUploader {
	pub(crate) fn new(sender: tokio::sync::watch::Sender<Option<(u64, bytes::Bytes)>>) -> Self {
		Self {
			sender,
		}
	}

	/// Queue a manifest upload. Always succeeds — overwrites any pending upload
	/// with the latest manifest (which is a complete snapshot, not a delta).
	pub(crate) fn queue_upload(&self, manifest_id: u64, data: Vec<u8>) {
		let _ = self.sender.send(Some((manifest_id, bytes::Bytes::from(data))));
	}
}
...
pub(crate) async fn load_from_bytes(
	...
	let writer_epoch = cursor.read_u64::<BigEndian>()?;
	let compactor_epoch = cursor.read_u64::<BigEndian>()?;
	let leader_id = cursor.read_u128::<BigEndian>()?;

	log::debug!(
		"Manifest header: version={}, manifest_id={}, log_number={}, last_sequence={}, writer_epoch={}, compactor_epoch={}, leader_id={}",
		version,
		manifest_id,
		log_number,
		last_sequence,
	...
	log::info!(
		"Manifest loaded: version={}, manifest_id={}, log_number={}, last_sequence={}, writer_epoch={}, compactor_epoch={}, leader_id={}, tables={}, levels={}",
		version, manifest_id, log_number, last_sequence, writer_epoch, compactor_epoch, leader_id, total_tables, levels_vec.len()
	);
...
pub(crate) fn serialize_manifest(manifest: &LevelManifest) -> Result<Vec<u8>> {
	...
	// Write header: version | manifest_id | log_number | last_sequence | writer_epoch |
	// compactor_epoch | leader_id
	...
	buf.write_u128::<BigEndian>(manifest.leader_id)?;
...
/// Reset the header fields of a serialized manifest for branch creation.
///
/// Takes an existing serialized manifest and rewrites the header so the branch
/// starts with manifest_id=1, writer_epoch=0, compactor_epoch=0, leader_id=0,
/// and log_number=0. The levels data and snapshots are preserved unchanged.
///
/// This avoids deserializing/re-serializing the full manifest and is safe because
/// the header layout is fixed at the start of the byte buffer.
pub(crate) fn reset_manifest_for_branch(data: &[u8]) -> Result<Vec<u8>> {
	if data.len() < 2 + 8 + 8 + 8 + 8 + 8 + 16 {
		return Err(Error::LoadManifestFail("Manifest too short for header reset".to_string()));
	}
	let mut buf = data.to_vec();
	let mut cursor = std::io::Cursor::new(&mut buf[..]);

	// Skip version (2 bytes) — keep as-is
	cursor.set_position(2);
	// manifest_id = 1
	cursor.write_u64::<BigEndian>(1)?;
	// log_number = 0 (branch has no WAL history)
	cursor.write_u64::<BigEndian>(0)?;
	// last_sequence — keep as-is (skip 8 bytes)
	...
	cursor.write_u64::<BigEndian>(0)?;
	// leader_id = 0
	cursor.write_u128::<BigEndian>(0)?;

	Ok(buf)
}

/// Extract the set of SST IDs from a serialized manifest without fully loading tables.
///
/// This reads the levels encoding to extract table IDs for GC purposes,
/// without opening the actual SST files from object store.
pub(crate) fn extract_sst_ids_from_manifest(data: &[u8]) -> Result<HashSet<SstId>> {
	use crate::levels::level::Levels;
	...
	// Skip header fields: manifest_id(8) + log_number(8) + last_sequence(8)
	// + writer_epoch(8) + compactor_epoch(8) + leader_id(16)
	cursor.set_position(2 + 8 + 8 + 8 + 8 + 8 + 16);

	let level_data = Levels::decode(&mut cursor)?;
	let mut ids = HashSet::new();
	for table_entries in level_data {
		for entry in table_entries {
			ids.insert(entry.id);
		}
	}
...
pub(crate) fn write_manifest_to_disk(manifest: &mut LevelManifest) -> Result<Vec<u8>> {
	...
	log::debug!(
		"Writing manifest: version={}, manifest_id={}, log_number={}, last_sequence={}, writer_epoch={}, compactor_epoch={}, leader_id={}, total_tables={}",
	...
	for entry in entries.flatten() {
		...
		if let Some(id_str) = name_str.strip_suffix(".manifest") {
			if let Ok(id) = id_str.parse::<u64>() {
				if latest.as_ref().is_none_or(|(best, _)| id > *best) {
